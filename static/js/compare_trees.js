...
function updateTree(source,viewOptions) {
  ...
  nodeEnter.append("circle")
    ...
    .attr('data-original-title', function(d) { return d.name+"<br>Z-Score: "+Math.round(10*d.zscores[currentFile])/10; })
    .attr("class","my_node");
  ...
  nodeUpdateTree.select("circle")
    ...
    .attr('data-original-title', function(d) { 
      return d.name+"<br>z-score: "+Math.round(10*d.zscores[currentFile])/10; 
    })
    .attr("class","my_node");
...
function plotBarChart(elm, d, i) {
  ...
  $('#compareFileNames').children().each( function() {
    var name = this.value;
    names.push(name);
...
/**  for (name in d.zscores) {
  ...
  barGroup.append("svg:foreignObject")
    .attr("width", 10)
    .attr("height", 10)
    .attr("x","120px")
    .attr("y","-40px")
    .attr("class","close-bar-chart glyphicon glyphicon-remove")
    .style("font-size","9px")
    .on("click", function() {
      $(".barGraph").fadeOut(300, function() { $(this).remove(); })
      $(".barChartBackground").fadeOut(300, function() { $(this).remove(); })
    });
  ...
  barEnter.append("rect")
    ...
    .classed('active', function(d) { 
      if (d.name == currentFile) {
        return true;
      } 
      return false;
    })
    .on("click",function(d) {
      $('rect.bar.active').each( function() {
        this.classList.remove('active');
      })
      $('#compareFileNames').val(d.name);
      $('#compareFileNames').change();
      this.classList.add("active");
    });
    
  barEnter.selectAll("rect").transition().duration(300)
    .attr("width", function(d) { return Math.abs(xScale(d.value) - xScale(0)); });
    
  barEnter.append("text")
    .text(function(d) { return Math.round(d.value*10)/10 })
    .attr("x", function(d) { return xScale(Math.max(0,d.value)); })
    .attr("y", function(d) { return yScale(d.name); })
    .attr("dy", "1.55em")
    .attr("dx", "1px")
    .style("font-size","6px")
